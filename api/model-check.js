...
function setCors(res) {
  ...
  res.setHeader("X-Model-Check-Version", "provider-aware-v3-debug");
}
...
function buildRequestDebug(req, body, options = {}) {
  ...
  const apiKeyInfo = options.apiKeyInfo || getApiKeyInfo(req);
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    return sendJson(res, 200, {
      ok: true,
      service: "model-check",
      version: "provider-aware-v3-debug",
  ...
  try {
    ...
    const apiKeyInfo = getApiKeyInfo(req);
