...
const PARSE_VERSION = 'parse-url-clean-deep-probe-v5-20260621';
...
function getExtension(resourceUrl) {
  try {
    const cleanedUrl = cleanInputUrl(resourceUrl);
...
function absolutizeUrl(rawUrl, baseUrl) {
  ...
  try {
    const cleanedBaseUrl = cleanInputUrl(baseUrl);
    return new URL(cleaned, cleanedBaseUrl).toString();
  } catch {
    return null;
  }
}
...
function getSameOriginPageLinks(html, baseUrl, options = {}) {
  ...
  while ((match = linkRegex.exec(html)) !== null) {
    ...
    try {
      const cleanedBaseUrl = cleanInputUrl(baseUrl);
      const base = new URL(cleanedBaseUrl);
      const u = new URL(abs);
...
async function fetchWithTimeout(url, options = {}, timeoutMs = 8000) {
  ...
  try {
    ...
    throw new Error(`Invalid fetch url after cleaning: ${cleanedUrl}`);
  }

  if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
    throw new Error(`Blocked non-http fetch url: ${cleanedUrl}`);
  }

  if (isPrivateOrLocalUrl(cleanedUrl)) {
    throw new Error(`Blocked private or local fetch url: ${cleanedUrl}`);
  }

  console.log('[fetchWithTimeout] inputUrl:', url);
  console.log('[fetchWithTimeout] cleanedUrl:', cleanedUrl);

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);

  try {
    return await fetch(cleanedUrl, {
      ...options,
...
async function probeResource(resourceUrl) {
  const cleanedResourceUrl = cleanInputUrl(resourceUrl);
  ...
  try {
    ...
    return probeFromHeaders(cleanedResourceUrl, resp.status, resp.headers);
  } catch {
    return null;
  }
}
...
async function probeCandidateResources(urls, options = {}) {
  ...
  for (const u of candidates) {
    ...
    if (info) {
      probeMap.set(cleanInputUrl(u), info);
      probeMap.set(u, info);
    }
  }
...
function filterResources(urls, options = {}) {
  ...
  for (const resourceUrlRaw of urls) {
    const resourceUrl = cleanInputUrl(resourceUrlRaw);
    const probeInfo =
      probeMap.get(resourceUrl) || probeMap.get(resourceUrlRaw) || null;
...
async function fetchPageHtml(pageUrl) {
  const cleanedPageUrl = cleanInputUrl(pageUrl);

  console.log('[fetchPageHtml] input:', pageUrl);
  console.log('[fetchPageHtml] cleaned:', cleanedPageUrl);
  ...
  if (!isHtml) {
    return {
      ...
      html: '',
      directResource: true,
      probeInfo: probeFromHeaders(cleanedPageUrl, resp.status, resp.headers),
    };
  }
...
async function handleParse(req, res, options) {
  ...
  try {
    ...
    allUrls = [...new Set(allUrls.map((u) => cleanInputUrl(u)).filter(Boolean))];
