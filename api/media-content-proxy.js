...
const PROXY_VERSION = "lingche-media-content-proxy-v46.3-final-from-v34.2";
...
const ALLOWED_MEDIA_HOSTS = (process.env.ALLOWED_MEDIA_HOSTS || DEFAULT_ALLOWED_MEDIA_HOSTS.join(","))
  .split(",")
...
function setCors(res, extra = {}) {
  ...
  res.setHeader("X-Lingche-Backend-Version", PROXY_VERSION);
...
function isAllowedMediaUrl(targetUrl) {
  try {
    ...
    if (ALLOW_ANY_HTTPS_MEDIA && url.protocol === "https:") return true;

    return ALLOWED_MEDIA_HOSTS.some((allowed) => hostMatchesAllowed(url.hostname, allowed));
  } catch {
    return false;
  }
}
...
function pickTargetFromBody(body) {
  return body.url || body.u || body.mediaUrl || body.fileUrl || body.downloadUrl || body.videoUrl || body.imageUrl || "";
}
...
function buildForwardHeaders(req, body = {}) {
  ...
  if (ifRange) headers["If-Range"] = Array.isArray(ifRange) ? ifRange[0] : ifRange;

  const userAgent = req.headers["user-agent"];
  headers["User-Agent"] = Array.isArray(userAgent) ? userAgent[0] : userAgent || "Lingche-Media-Proxy/46.3";
  headers.Accept = req.headers.accept || "*/*";

  const referer = req.headers.referer || req.headers.referrer;
  if (referer) headers.Referer = Array.isArray(referer) ? referer[0] : referer;
  ...
  if (authHeader) headers.Authorization = Array.isArray(authHeader) ? authHeader[0] : authHeader;

  const envKey = process.env.UPSTREAM_API_KEY || "";
  const bodyKey = body.apiKey || body.upstreamApiKey || "";
  const finalKey = envKey || bodyKey;

  if (finalKey && !headers.Authorization) headers.Authorization = `Bearer ${finalKey}`;
...
export default async function handler(req, res) {
  ...
  if (req.method === "HEAD" && !getTargetUrlFromQuery(req)) return res.status(200).end();
  if (req.method === "GET" && !getTargetUrlFromQuery(req)) return sendHealth(res);
  ...
  try {
    ...
    const rawTarget = req.method === "POST" ? pickTargetFromBody(body) : getTargetUrlFromQuery(req);
    ...
    if (!isAllowedMediaUrl(targetUrl)) {
      return sendJson(res, 403, {
        ...
        allowedMediaHosts: ALLOWED_MEDIA_HOSTS,
        allowAnyHttpsMedia: ALLOW_ANY_HTTPS_MEDIA,
        hint: "如果视频/图片来自第三方 CDN，请把域名加入 ALLOWED_MEDIA_HOSTS，或测试时设置 ALLOW_ANY_HTTPS_MEDIA=true。",
      });
    }
    ...
    if (contentTypeLower.includes("text/html")) {
      const preview = req.method === "HEAD" ? "" : (await upstream.text()).slice(0, 1000);
      return sendJson(res, upstream.status || 502, {
        ok: false,
        success: false,
        error: "媒体地址返回 HTML，不是图片/视频/文件。可能是链接过期、鉴权失败、被防盗链拦截或地址不是直链。",
    ...
    return sendJson(res, 500, {
      ...
      hint: "media-content-proxy 处理失败。请检查媒体 URL、白名单、鉴权、Range 请求和上游 CDN 状态。",
    });
  }
}
